...
+!perform_look_action
    <-  // Aimed agents have the following format:
        ...
        .length(ObjList, L);
        +targeted_agent(-1);
        while (i(Current) & Current < L & targeted_agent(Cod) & Cod == -1) {
          .nth(Current, ObjList, Obj);
          .nth(1, Obj, Team);
          if (Team == 100) {
            -+targeted_agent(Current);
          }
          -+i(Current + 1);
        }

        if (targeted_agent(Objective) & Objective >= 0) {
          .nth(Objective, ObjList, Target);
